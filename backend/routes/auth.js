...
function logSessionCookie(req, context) {
  if (process.env.DEBUG_LOG_SESSION_COOKIE !== "true") {
    return;
  }

  const sessionCookieValue = req.cookies?.sid || req.sessionID;

  if (!sessionCookieValue) {
    console.log(`[auth debug] ${context}: no session cookie available`);
    return;
  }

  console.log(`[auth debug] ${context}: sid=${sessionCookieValue}`);
}
